...
/*
 * This is taken directly from Umbraco.Cms.Core.Composing.ComposerGraph.cs
 */
...
/// </summary>
public class UHeadlessComposerGraph
{
    ...
    private readonly ILogger<UHeadlessComposerGraph> _logger;

    /// <summary>
    ///     Initializes a new instance of the <see cref="ComposerGraph" /> class.
    /// </summary>
    /// <param name="builder">The composition.</param>
    /// <param name="composerTypes">The <see cref="IUHeadlessComposer" /> types.</param>
    ...
    ///     logger
    /// </exception>
    public UHeadlessComposerGraph(IUmbracoBuilder builder, IEnumerable<Type> composerTypes, IEnumerable<Attribute> enableDisableAttributes, ILogger<UHeadlessComposerGraph> logger)
    {
    ...
    public void Compose()
    {
        ...
        foreach (IUHeadlessComposer composer in InstantiateComposers(orderedComposerTypes))
        {
    ...
    internal Dictionary<Type, List<Type>?> GetRequirements(bool throwOnMissing = true)
    {
            ...
            where TAttribute : Attribute
        {
            foreach (TAttribute attribute in type.GetCustomAttributes<TAttribute>())
            {
                ...
                if (typeInAttribute != null && // if the attribute references a type ...
                    ...
                    typeof(IUHeadlessComposer).IsAssignableFrom(typeInAttribute) && // ... which implements IUHeadlessComposer ...
                    !iset.Contains(typeInAttribute)) // ... which is not already in the list
                {
                    ...
                    // add all its interfaces implementing IUHeadlessComposer
                    foreach (Type i in typeInAttribute.GetInterfaces()
                                 .Where(x => typeof(IUHeadlessComposer).IsAssignableFrom(x)))
                    {
                        iset.Add(i);
                        set2.Add(i);
                    }
        ...
        var interfaces = new HashSet<Type>(composerTypeList.SelectMany(x =>
            x.GetInterfaces().Where(y => typeof(IUHeadlessComposer).IsAssignableFrom(y))));
    ...
    private static IEnumerable<IUHeadlessComposer> InstantiateComposers(IEnumerable<Type> types)
    {
        foreach (Type type in types)
        {
            ...
            yield return (IUHeadlessComposer) ctor.Invoke(Array.Empty<object>());
        }
    }
